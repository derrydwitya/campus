...
for($i=0; $i<count($daftar_kampus); $i++){
	for($j=count($daftar_kampus)-1; $j>0; $j--){
		if($daftar_kampus[$j]->get_jarak()<$daftar_kampus[$j-1]->get_jarak()){
			$temp=$daftar_kampus[$j];
			$daftar_kampus[$j]=$daftar_kampus[$j-1];
			$daftar_kampus[$j-1]=$temp;
		}
	}
}
